  ...
  try {
    ...
    if (ENABLE_GUI) {
      ...
      app.get("/api/tasks", async (req: Request, res: Response) => {
        try {
          // Use fsPromises to keep async reading
          const tasksData = await fsPromises.readFile(TASKS_FILE_PATH, "utf-8");
          res.json(JSON.parse(tasksData));
        } catch (error) {
          // Ensure empty task list is returned if file doesn't exist
      ...
      const httpServer = app.listen(port, () => {
        // Start listening to file changes after server starts
        try {
          // Check if file exists, do not watch if it doesn't (to avoid watch errors)
          if (fs.existsSync(TASKS_FILE_PATH)) {
            fs.watch(TASKS_FILE_PATH, (eventType, filename) => {
              if (
                ...
                // Slightly delay sending to prevent multiple triggers in a short time
      ...
      // Set up process termination event handling (ensure watcher is removed)
